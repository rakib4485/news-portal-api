const loadHeader = async (index)=>{
    ...
    li.innerHTML = `
        ...
        <a onclick ="loadBlogs('${category_id}','${itemData}')" id = "btn-${itemData}" class="nav-link active mx-2 rounded fs-5" style = "cursor: pointer; background-color : #FFEEAF">${itemData}</a>
    `;
    //li.id.add('${itemData}');
...
const displayBlog = (datas , itemData) =>{
    ...
    countDiv.innerHTML = `
      <div class="container p-3 my-5 " style="background-color: #F1A661; border-radius: 10px;">
        ...
        <div class="card mb-3" style = "border-radius: 20px;">
            <div class="row g-0">
                ...
                <div class="col-md-9">
                    ...
                    <p class="card-text text-elipsis">${index.details}</p>
                  ...
                  <div class="col-8 col-md-3 col-lg-4">
                    <button onclick = "loadBlogDetails('${index.category_id}', '${index._id}')" class="btn btn-primary"  data-bs-toggle="modal"  data-bs-target="#blogDetailsModal">Click Me <i class="fa-solid fa-right-long"></i></button>
                  </div>
                </div>
                    </div>
                </div>
